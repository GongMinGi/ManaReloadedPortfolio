...
public class EnemyHealthBarUI : HealthBarBinder
{
    ...
    [SerializeField] private Vector3 worldOffset = new Vector3(0, 1.0f, 0);     // 적 위치에서 조금 위쪽에 표시
    ...
    private void LateUpdate()
    {
        ...
        if (followingPos) transform.position = followingPos.position + worldOffset;         // 기준 위치가 유효하면, worldOffset 만큼 머리 위로 올려서 배치
    ...
    protected override void OnRatioChanged(float ratio)     // 체력 비율이 바뀔때마다 호출
    {
        ...
        if (hideWhenFull)                           // 풀피 숨김 옵션이 true일때만
